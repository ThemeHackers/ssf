...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const inputs = ['target-url', 'target-key', 'ai-provider', 'ai-model', 'ai-key', 'scan-level',
...
function handleConnectionError() {
    ...
    if (connectionFailures >= 3) {
        ...
        if (localStorage.getItem('ssf_report')) {
            ...
            document.querySelector('.status-pill span').textContent = "Disconnected";
            ...
            el.innerHTML = `
                ...
                <div class="exploit-body">
                    ...
                    <div class="exploit-meta clickable" onclick='openEditPayloadModal("${target}", ${payloadStr}, "${exp.type}")' title="Click to edit payload">
                        <span class="meta-item"><strong>Payload:</strong> ${JSON.stringify(payload)} <span style="float:right">✏️</span></span>
                    </div>
                </div>
